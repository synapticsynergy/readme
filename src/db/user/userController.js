...
User.prototype.addActivity = function(activities, day, location) {
  var user = this;
  return user.addPopular('activities', activities).then(function(user) {
...
User.prototype.deleteActivity = function(activity, day) {
  var user = this;
  return user.getDay(day).then(function(foundDay) {
    foundDay.activities.splice(foundDay.activities.indexOf(activity), 1);
    return user.save();
  });
};
User.prototype.addMetric = function(metrics, day) {
  ...
  var user = this;
  user.addPopular('metrics', metrics).then(function(user) {
    return user.getDay(day);
  }).then(function(foundDay) {
...
User.prototype.deleteMetric = function(metric, day) {
  var user = this;
  return user.getDay(day).then(function(foundDay) {
    foundDay.metrics.splice(foundDay.metrics.indexOf(metric), 1);
    return user.save();
  });
};
...
User.prototype.addSentiment = function(date, sentiment) {
  var user = this;
  return user.getDay(date).then(function(day) {
...
User.prototype.getWeather = function(date, location, activities, foundDay) {
  ...
  return http(options).then(function(weatherData) {
